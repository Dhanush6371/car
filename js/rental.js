...
document.addEventListener("DOMContentLoaded", () => {
  const elementsToAnimate = document.querySelectorAll(".lazy");

  const observer = new IntersectionObserver(
    (entries, observer) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          const animationClass = entry.target.classList.contains("animate-from-right")
            ? "slideInFromRight"
            : "slideInFromLeft";
          entry.target.style.animation = `${animationClass} 1s ease-in-out forwards`;
